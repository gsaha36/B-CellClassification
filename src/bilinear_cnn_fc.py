...
def OD_init(input_shape, ref_img_path):
    ...
    query = mpimg.imread(ref_img_path) / 255.0
...
class BCNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        PHI = OD_init((3,3,1,1),'./ref_all.bmp')
        W = torch.from_numpy(PHI).float().to('cuda')
        #self.sdlayer = SDLayer(W)
        # Convolution and pooling layers of VGG-16.
        self.features = torchvision.models.alexnet(pretrained=True).features
        self.features = torch.nn.Sequential(*list(self.features.children())
                                            [:-1])  # Remove pool5.
        # Linear classifier.
        self.fc = torch.nn.Linear(256**2, 200)#512
        #self.fc = torch.nn.Linear(256*256*24*24, 200)#512
    ...
    def forward(self, X):
        ...
        """
        N = X.size()[0]
        assert X.size() == (N, 3, 400, 400) #448,448
        #X = self.sdlayer(X)
        X = self.features(X)
        #print(X.size())
        assert X.size() == (N, 256, 24, 24)#N,512,28,28
        X = X.view(N, 256, 24**2)
...
class BCNNManager(object):
    ...
    def __init__(self, options, path):
        ...
        train_transforms = torchvision.transforms.Compose([
            torchvision.transforms.Resize(size=400),  # Let smaller edge match
            torchvision.transforms.RandomHorizontalFlip(),
            torchvision.transforms.RandomVerticalFlip(),
            torchvision.transforms.RandomRotation(180),
            torchvision.transforms.RandomCrop(size=400),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                             std=(0.229, 0.224, 0.225))
        ])
        test_transforms = torchvision.transforms.Compose([
            torchvision.transforms.Resize(size=400),
            torchvision.transforms.CenterCrop(size=400),
            torchvision.transforms.ToTensor(),
            torchvision.transforms.Normalize(mean=(0.485, 0.456, 0.406),
                                             std=(0.229, 0.224, 0.225))
        ])
        ...
        train_data = datasets.ImageFolder(root='../cell_dataset',
                                                   transform=train_transforms)
       	test_data = datasets.ImageFolder(root='../cell_test',
                                          transform=test_transforms)
    ...
    def train(self):
        ...
        for t in range(self._options['epochs']):
            ...
            for X, y in self._train_loader:
                ...
                num_total += y.size(0)
                num_correct += torch.sum(prediction == y.data).cpu().numpy()
                # Backward pass.
                loss.backward()
                self._solver.step()
